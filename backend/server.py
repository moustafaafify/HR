...
class HeadcountPlan(BaseModel):
    """Headcount planning for departments/roles"""
    model_config = ConfigDict(extra="ignore")
    id: str = Field(default_factory=lambda: str(uuid.uuid4()))
    name: str
    description: Optional[str] = None
    ...
    # Breakdown by role
    role_breakdown: List[Dict[str, Any]] = Field(default_factory=list)  # [{role, current, target, priority}]
...
class SkillGap(BaseModel):
    ...
    skill_name: str
    skill_category: str = "technical"  # technical, soft, leadership, domain
    
    # Gap metrics
    required_level: int = 3  # 1-5 scale
    current_avg_level: float = 0
    gap_score: float = 0  # required - current
    
    # Employees with skill
    employees_with_skill: int = 0
    employees_needed: int = 0
    
    # Priority and action
    priority: str = "medium"  # low, medium, high, critical
    recommended_action: str = "training"  # training, hire, outsource, promote
...
class ResourceAllocation(BaseModel):
    """Employee allocation to projects/teams"""
    model_config = ConfigDict(extra="ignore")
    id: str = Field(default_factory=lambda: str(uuid.uuid4()))
    employee_id: str
    employee_name: Optional[str] = None
...
class CapacityPlan(BaseModel):
    """Team/department capacity planning"""
    model_config = ConfigDict(extra="ignore")
    id: str = Field(default_factory=lambda: str(uuid.uuid4()))
    name: str
    department_id: Optional[str] = None
    ...
    # Capacity metrics
    total_fte: float = 0  # Full-time equivalents
    available_hours: float = 0
    allocated_hours: float = 0
    utilization_target: float = 80  # Target utilization %
    current_utilization: float = 0
    
    # Workload
    planned_projects: List[Dict[str, Any]] = Field(default_factory=list)  # [{project_id, name, hours_required}]
    
    # Risks
    over_capacity: bool = False
    under_capacity: bool = False
...
class WorkforceScenario(BaseModel):
    ...
    scenario_type: str = "growth"  # growth, reduction, restructure, merger
    ...
    # Scenario parameters
    changes: List[Dict[str, Any]] = Field(default_factory=list)  # [{type, department, count, cost_impact}]
    ...
    # Impact analysis
    productivity_impact: str = "neutral"  # negative, neutral, positive
    morale_impact: str = "neutral"
    risk_assessment: Optional[str] = None
    
    status: str = "draft"  # draft, under_review, approved, rejected, implemented
...
class EmployeeAvailability(BaseModel):
    """Employee availability for resource planning"""
    model_config = ConfigDict(extra="ignore")
    id: str = Field(default_factory=lambda: str(uuid.uuid4()))
    employee_id: str
    ...
    # Availability status
    status: str = "available"  # available, partial, unavailable, on_leave
    reason: Optional[str] = None
    
    # Preferences (for employees to set)
...
class CompliancePolicy(BaseModel):
    ...
    title: str
    description: Optional[str] = None
    category: str = "general"  # general, hr, safety, data_privacy, security, financial, ethics
    ...
    acknowledgement_frequency: str = "once"  # once, annual, quarterly
    applicable_departments: List[str] = Field(default_factory=list)  # empty = all
    applicable_roles: List[str] = Field(default_factory=list)  # empty = all
...
class ComplianceTraining(BaseModel):
    ...
    title: str
    description: Optional[str] = None
    category: str = "general"  # general, safety, security, harassment, ethics, data_privacy
    
    # Content
    content_type: str = "video"  # video, document, quiz, external_link
...
class TrainingCompletion(BaseModel):
    ...
    # Progress
    status: str = "assigned"  # assigned, in_progress, completed, failed, expired
...
class LegalDocument(BaseModel):
    ...
    id: str = Field(default_factory=lambda: str(uuid.uuid4()))
    title: str
    document_type: str = "contract"  # contract, nda, agreement, amendment, offer_letter, termination
    description: Optional[str] = None
...
class ComplianceIncident(BaseModel):
    ...
    title: str
    description: str
    incident_type: str = "policy_violation"  # policy_violation, safety, harassment, data_breach, ethics, fraud
    ...
    # Investigation
    status: str = "reported"  # reported, under_investigation, resolved, escalated, closed
...
class ComplianceCertification(BaseModel):
    ...
    certification_type: str = "internal"  # internal, external, regulatory
...
class Visitor(BaseModel):
    ...
    id_type: Optional[str] = None  # drivers_license, passport, national_id, employee_id
    id_number: Optional[str] = None
    
    # Visit details
    visit_type: str = "meeting"  # meeting, interview, delivery, contractor, event, other
    ...
    expected_time: Optional[str] = None  # HH:MM
    expected_duration_minutes: int = 60
    
    # Status
    status: str = "pre_registered"  # pre_registered, checked_in, checked_out, cancelled, no_show
...
class VisitorBadge(BaseModel):
    """Visitor badge template and configuration"""
    model_config = ConfigDict(extra="ignore")
    id: str = Field(default_factory=lambda: str(uuid.uuid4()))
    ...
    # Badge design
    badge_type: str = "standard"  # standard, contractor, event, vip
...
class VisitorSettings(BaseModel):
    ...
    # Pre-registration
    allow_employee_registration: bool = True
    require_approval: bool = False
    advance_registration_days: int = 30
    
    # Check-in
    require_photo: bool = False
    require_id: bool = True
    require_nda: bool = False
    auto_notify_host: bool = True
    
    # Badge
    badge_prefix: str = "V"
    badge_expiry_hours: int = 12
    
    # Notifications
    notify_on_checkin: bool = True
    notify_on_checkout: bool = False
    
    # Visit types enabled
    enabled_visit_types: List[str] = Field(default_factory=lambda: ["meeting", "interview", "delivery", "contractor", "event", "other"])
...
async def create_visitor(data: Dict[str, Any], current_user: User = Depends(get_current_user)):
    ...
    # Get the registering user's employee info
    ...
    # If no host specified and user is not admin, set self as host
...
async def get_visitors(
    ...
    # Non-admins can only see their own visitors
    if current_user.role not in [UserRole.SUPER_ADMIN, UserRole.CORP_ADMIN, UserRole.BRANCH_MANAGER]:
        emp = await db.employees.find_one({"user_id": current_user.id}, {"_id": 0})
        if emp:
            query["$or"] = [
...
# --- Dashboard/Stats (MUST be before /{visitor_id} routes to avoid path parameter conflicts) ---
@api_router.get("/visitors/dashboard")
async def get_visitors_dashboard(current_user: User = Depends(get_current_user)):
    ...
    # Visit types breakdown for today
...
@api_router.get("/visitors/my-dashboard")
...
@api_router.get("/visitors/history")
...
async def update_visitor(visitor_id: str, data: Dict[str, Any], current_user: User = Depends(get_current_user)):
    ...
    # Update host info if host changed
...
# --- Badge ---
@api_router.post("/visitors/{visitor_id}/print-badge")
...
@api_router.get("/visitors/{visitor_id}/badge")
...
@api_router.post("/visitors/walk-in")
...
# --- Policies ---
@api_router.post("/compliance/policies")
...
@api_router.get("/compliance/policies")
...
@api_router.get("/compliance/policies/{policy_id}")
...
@api_router.put("/compliance/policies/{policy_id}")
...
@api_router.delete("/compliance/policies/{policy_id}")
async def delete_policy(policy_id: str, current_user: User = Depends(get_current_user)):
    ...
    result = await db.compliance_policies.delete_one({"id": policy_id})
    if result.deleted_count == 0:
        raise HTTPException(status_code=404, detail="Policy not found")
    return {"message": "Policy deleted"}

# --- Policy Acknowledgements ---
@api_router.post("/compliance/policies/{policy_id}/acknowledge")
async def acknowledge_policy(policy_id: str, current_user: User = Depends(get_current_user)):
    ...
    # Calculate valid_until based on acknowledgement_frequency
    valid_until = None
    freq = policy.get("acknowledgement_frequency", "once")
    if freq == "annual":
...
@api_router.get("/compliance/policies/{policy_id}/acknowledgements")
async def get_policy_acknowledgements(policy_id: str, current_user: User = Depends(get_current_user)):
    """Get all acknowledgements for a policy (admin only)"""
    if current_user.role not in [UserRole.SUPER_ADMIN, UserRole.CORP_ADMIN]:
        raise HTTPException(status_code=403, detail="Only admins can view all acknowledgements")
    
    acks = await db.policy_acknowledgements.find({"policy_id": policy_id}, {"_id": 0}).to_list(1000)
    return acks

@api_router.get("/compliance/my-acknowledgements")
async def get_my_acknowledgements(current_user: User = Depends(get_current_user)):
    """Get current user's policy acknowledgements"""
    emp = await db.employees.find_one({"user_id": current_user.id}, {"_id": 0})
    if not emp:
        return []
    
    acks = await db.policy_acknowledgements.find({"employee_id": emp["id"]}, {"_id": 0}).to_list(200)
    return acks

# --- Compliance Training ---
@api_router.post("/compliance/trainings")
...
@api_router.get("/compliance/trainings")
async def get_trainings(
    status: Optional[str] = None,
    category: Optional[str] = None,
    current_user: User = Depends(get_current_user)
):
    """Get all compliance trainings"""
    query = {}
    if status:
        query["status"] = status
    if category:
...
@api_router.get("/compliance/trainings/{training_id}")
...
@api_router.put("/compliance/trainings/{training_id}")
...
@api_router.delete("/compliance/trainings/{training_id}")
async def delete_training(training_id: str, current_user: User = Depends(get_current_user)):
    ...
    result = await db.compliance_trainings.delete_one({"id": training_id})
    if result.deleted_count == 0:
        raise HTTPException(status_code=404, detail="Training not found")
    return {"message": "Training deleted"}

# --- Training Assignments/Completions ---
@api_router.post("/compliance/trainings/{training_id}/assign")
async def assign_training(training_id: str, data: Dict[str, Any], current_user: User = Depends(get_current_user)):
    ...
    employee_ids = data.get("employee_ids", [])
    validity_months = training.get("certification_validity_months", 12)
    expires_at = (datetime.now(timezone.utc) + timedelta(days=validity_months * 30)).isoformat()
    ...
    for emp_id in employee_ids:
        ...
        created.append(completion.model_dump())
    
    return {"assigned": len(created), "completions": created}

@api_router.get("/compliance/trainings/{training_id}/completions")
async def get_training_completions(training_id: str, current_user: User = Depends(get_current_user)):
    """Get all completions for a training (admin only)"""
    if current_user.role not in [UserRole.SUPER_ADMIN, UserRole.CORP_ADMIN]:
        raise HTTPException(status_code=403, detail="Only admins can view all completions")
    
    completions = await db.training_completions.find({"training_id": training_id}, {"_id": 0}).to_list(1000)
    return completions

@api_router.get("/compliance/my-trainings")
async def get_my_trainings(current_user: User = Depends(get_current_user)):
    ...
    completions = await db.training_completions.find({"employee_id": emp["id"]}, {"_id": 0}).to_list(200)
    return completions

@api_router.put("/compliance/my-trainings/{completion_id}")
async def update_my_training_progress(completion_id: str, data: Dict[str, Any], current_user: User = Depends(get_current_user)):
    """Update training progress"""
    emp = await db.employees.find_one({"user_id": current_user.id}, {"_id": 0})
    if not emp:
        raise HTTPException(status_code=404, detail="Employee profile not found")
    
    completion = await db.training_completions.find_one({"id": completion_id, "employee_id": emp["id"]}, {"_id": 0})
    if not completion:
        raise HTTPException(status_code=404, detail="Training assignment not found")
    
    update_data = {}
    if "progress_percentage" in data:
        update_data["progress_percentage"] = data["progress_percentage"]
    if "status" in data:
        update_data["status"] = data["status"]
        if data["status"] == "in_progress" and not completion.get("started_at"):
            update_data["started_at"] = datetime.now(timezone.utc).isoformat()
        if data["status"] == "completed":
            update_data["completed_at"] = datetime.now(timezone.utc).isoformat()
    if "score" in data:
        update_data["score"] = data["score"]
        update_data["attempts"] = completion.get("attempts", 0) + 1
    
    await db.training_completions.update_one({"id": completion_id}, {"$set": update_data})
    return await db.training_completions.find_one({"id": completion_id}, {"_id": 0})

# --- Legal Documents ---
@api_router.post("/compliance/documents")
async def create_legal_document(data: Dict[str, Any], current_user: User = Depends(get_current_user)):
...
async def get_legal_documents(
    ...
    docs = await db.legal_documents.find(query, {"_id": 0}).sort("created_at", -1).to_list(500)
    return docs

@api_router.get("/compliance/documents/{doc_id}")
async def get_legal_document(doc_id: str, current_user: User = Depends(get_current_user)):
    ...
    if current_user.role not in [UserRole.SUPER_ADMIN, UserRole.CORP_ADMIN]:
        emp = await db.employees.find_one({"user_id": current_user.id}, {"_id": 0})
        if not emp or doc.get("employee_id") != emp["id"]:
            raise HTTPException(status_code=403, detail="Access denied")
    
    return doc

@api_router.put("/compliance/documents/{doc_id}")
async def update_legal_document(doc_id: str, data: Dict[str, Any], current_user: User = Depends(get_current_user)):
    """Update a legal document"""
    if current_user.role not in [UserRole.SUPER_ADMIN, UserRole.CORP_ADMIN]:
        raise HTTPException(status_code=403, detail="Only admins can update documents")
    
    data["updated_at"] = datetime.now(timezone.utc).isoformat()
    await db.legal_documents.update_one({"id": doc_id}, {"$set": data})
    doc = await db.legal_documents.find_one({"id": doc_id}, {"_id": 0})
    return doc

@api_router.post("/compliance/documents/{doc_id}/sign")
async def sign_document(doc_id: str, current_user: User = Depends(get_current_user)):
    """Sign a legal document (employee signature)"""
    doc = await db.legal_documents.find_one({"id": doc_id}, {"_id": 0})
    if not doc:
        raise HTTPException(status_code=404, detail="Document not found")
    ...
    emp = await db.employees.find_one({"user_id": current_user.id}, {"_id": 0})
    if not emp or doc.get("employee_id") != emp["id"]:
        raise HTTPException(status_code=403, detail="You can only sign documents assigned to you")
    ...
    if doc.get("requires_company_signature") and doc.get("company_signed_at"):
        update_data["status"] = "signed"
    elif not doc.get("requires_company_signature"):
        update_data["status"] = "signed"
    else:
...
@api_router.delete("/compliance/documents/{doc_id}")
...
@api_router.post("/compliance/incidents")
async def create_incident(data: Dict[str, Any], current_user: User = Depends(get_current_user)):
...
@api_router.get("/compliance/incidents")
async def get_incidents(
    ...
    # Non-admins can only see incidents they reported
    if current_user.role not in [UserRole.SUPER_ADMIN, UserRole.CORP_ADMIN]:
        query["reported_by"] = current_user.id
    
    incidents = await db.compliance_incidents.find(query, {"_id": 0}).sort("reported_at", -1).to_list(500)
    return incidents

@api_router.get("/compliance/incidents/{incident_id}")
async def get_incident(incident_id: str, current_user: User = Depends(get_current_user)):
    ...
    # Non-admins can only view incidents they reported
    if current_user.role not in [UserRole.SUPER_ADMIN, UserRole.CORP_ADMIN]:
        if incident.get("reported_by") != current_user.id:
            raise HTTPException(status_code=403, detail="Access denied")
    
    return incident

@api_router.put("/compliance/incidents/{incident_id}")
...
# --- Certifications ---
@api_router.post("/compliance/certifications")
...
async def get_certifications(
    ...
    certs = await db.compliance_certifications.find(query, {"_id": 0}).sort("expiry_date", 1).to_list(500)
    return certs

@api_router.put("/compliance/certifications/{cert_id}")
async def update_certification(cert_id: str, data: Dict[str, Any], current_user: User = Depends(get_current_user)):
    ...
    cert = await db.compliance_certifications.find_one({"id": cert_id}, {"_id": 0})
    return cert

@api_router.delete("/compliance/certifications/{cert_id}")
async def delete_certification(cert_id: str, current_user: User = Depends(get_current_user)):
    ...
    result = await db.compliance_certifications.delete_one({"id": cert_id})
    if result.deleted_count == 0:
...
async def get_compliance_dashboard(current_user: User = Depends(get_current_user)):
    ...
    # Policy stats
    total_policies = await db.compliance_policies.count_documents({"status": "published"})
    
    # Get acknowledgement stats
    policies = await db.compliance_policies.find({"status": "published", "requires_acknowledgement": True}, {"_id": 0}).to_list(100)
    total_employees = await db.employees.count_documents({"status": "active"})
    
    pending_acknowledgements = 0
    for policy in policies:
        ack_count = await db.policy_acknowledgements.count_documents({"policy_id": policy["id"]})
        pending_acknowledgements += max(0, total_employees - ack_count)
    
    # Training stats
    total_trainings = await db.compliance_trainings.count_documents({"status": "active"})
    pending_trainings = await db.training_completions.count_documents({"status": {"$in": ["assigned", "in_progress"]}})
    completed_trainings = await db.training_completions.count_documents({"status": "completed"})
    overdue_trainings = await db.training_completions.count_documents({
        "status": {"$in": ["assigned", "in_progress"]},
        "expires_at": {"$lt": datetime.now(timezone.utc).isoformat()}
    })
    
    # Document stats
    pending_signatures = await db.legal_documents.count_documents({"status": "pending_signature"})
    total_documents = await db.legal_documents.count_documents({})
    
    # Incident stats
    open_incidents = await db.compliance_incidents.count_documents({"status": {"$in": ["reported", "under_investigation"]}})
    critical_incidents = await db.compliance_incidents.count_documents({"severity": "critical", "status": {"$nin": ["resolved", "closed"]}})
    
    # Certification stats
    expiring_soon = await db.compliance_certifications.count_documents({
        "status": "active",
        "expiry_date": {
    ...
    expired_certs = await db.compliance_certifications.count_documents({
        "expiry_date": {"$lt": datetime.now(timezone.utc).strftime("%Y-%m-%d")},
        "status": "active"
    })
    
    # Recent incidents
    recent_incidents = await db.compliance_incidents.find({}, {"_id": 0}).sort("reported_at", -1).to_list(5)
    
    # Policies needing review
    policies_expiring = await db.compliance_policies.find({
        "status": "published",
        "expiry_date": {
            "$gte": datetime.now(timezone.utc).strftime("%Y-%m-%d"),
            "$lte": (datetime.now(timezone.utc) + timedelta(days=30)).strftime("%Y-%m-%d")
    ...
    return {
        ...
        "policies_expiring": policies_expiring
    }

# --- Employee Compliance Overview ---
@api_router.get("/compliance/my-overview")
async def get_my_compliance_overview(current_user: User = Depends(get_current_user)):
    """Get current employee's compliance overview"""
    emp = await db.employees.find_one({"user_id": current_user.id}, {"_id": 0})
    if not emp:
        return {
            "policies": [],
    ...
    policies = await db.compliance_policies.find({"status": "published"}, {"_id": 0}).to_list(100)
    
    # Get my acknowledgements
    my_acks = await db.policy_acknowledgements.find({"employee_id": emp["id"]}, {"_id": 0}).to_list(200)
    acknowledged_ids = {a["policy_id"] for a in my_acks}
    
    # Pending policies
    pending_policies = [p for p in policies if p["id"] not in acknowledged_ids and p.get("requires_acknowledgement")]
    
    # My trainings
    my_trainings = await db.training_completions.find({"employee_id": emp["id"]}, {"_id": 0}).to_list(50)
    
    # My documents
    my_documents = await db.legal_documents.find({"employee_id": emp["id"]}, {"_id": 0}).sort("created_at", -1).to_list(50)
    
    # My certifications
    my_certifications = await db.compliance_certifications.find({"employee_id": emp["id"]}, {"_id": 0}).to_list(50)
    
    # Pending signatures
    pending_signatures = [d for d in my_documents if d.get("status") == "pending_signature" and not d.get("employee_signed_at")]
    
    return {
        "pending_policies": pending_policies,
        "acknowledged_policies": my_acks,
        "trainings": my_trainings,
        "documents": my_documents,
        "certifications": my_certifications,
        "pending_signatures": pending_signatures,
        "stats": {
            "pending_policies_count": len(pending_policies),
            "pending_trainings_count": len([t for t in my_trainings if t.get("status") in ["assigned", "in_progress"]]),
            "pending_signatures_count": len(pending_signatures),
            "active_certifications": len([c for c in my_certifications if c.get("status") == "active"])
        }
    }
...
# --- Headcount Plans ---
@api_router.post("/workforce/headcount-plans")
...
@api_router.get("/workforce/headcount-plans")
async def get_headcount_plans(
    ...
    plans = await db.headcount_plans.find(query, {"_id": 0}).sort("created_at", -1).to_list(100)
    return plans

@api_router.get("/workforce/headcount-plans/{plan_id}")
...
@api_router.put("/workforce/headcount-plans/{plan_id}")
...
@api_router.delete("/workforce/headcount-plans/{plan_id}")
async def delete_headcount_plan(plan_id: str, current_user: User = Depends(get_current_user)):
    ...
    result = await db.headcount_plans.delete_one({"id": plan_id})
    if result.deleted_count == 0:
        raise HTTPException(status_code=404, detail="Plan not found")
    return {"message": "Plan deleted"}

# --- Skills Gap Analysis ---
@api_router.get("/workforce/skills-gap")
async def get_skills_gap_analysis(
    ...
    gaps = await db.skill_gaps.find(query, {"_id": 0}).sort("gap_score", -1).to_list(100)
    return gaps

@api_router.post("/workforce/skills-gap/analyze")
async def analyze_skills_gap(data: Dict[str, Any], current_user: User = Depends(get_current_user)):
    """Run skills gap analysis for a department"""
    if current_user.role not in [UserRole.SUPER_ADMIN, UserRole.CORP_ADMIN]:
        raise HTTPException(status_code=403, detail="Only admins can run skills analysis")
    ...
    emp_query = {"department_id": department_id} if department_id else {}
    ...
    emp_skills = await db.employee_skills.find({"employee_id": {"$in": employee_ids}}, {"_id": 0}).to_list(5000)
    ...
    for skill in all_skills:
        ...
        employees_with_skill = [
            es for es in emp_skills 
            if es.get("skill_name", "").lower() == skill_name.lower() or es.get("skill_id") == skill.get("id")
        ]
        
        avg_level = 0
        if employees_with_skill:
            levels = [es.get("proficiency_level", es.get("level", 3)) for es in employees_with_skill]
            # Convert string levels to numbers
            level_map = {"beginner": 1, "intermediate": 2, "advanced": 3, "expert": 4, "master": 5}
            numeric_levels = []
            for l in levels:
                if isinstance(l, str):
                    numeric_levels.append(level_map.get(l.lower(), 3))
                else:
                    numeric_levels.append(int(l) if l else 3)
            avg_level = sum(numeric_levels) / len(numeric_levels) if numeric_levels else 0
        
        required_level = skill.get("required_level", 3)
        gap_score = max(0, required_level - avg_level)
        ...
        # Recommend action
        if len(employees_with_skill) == 0 and gap_score > 1:
            action = "hire"
        elif gap_score > 1.5:
            action = "training"
        else:
        ...
        skill_analysis[skill_name] = {
            ...
            "current_avg_level": round(avg_level, 2),
            "gap_score": round(gap_score, 2),
            "employees_with_skill": len(employees_with_skill),
            "employees_needed": max(0, 2 - len(employees_with_skill)) if gap_score > 1 else 0,
            "priority": priority,
            "recommended_action": action,
            "department_id": department_id,
            "department_name": dept.get("name") if dept else "All Departments"
        }
    
    # Save analysis results
    for skill_name, analysis in skill_analysis.items():
        gap = SkillGap(**analysis)
        await db.skill_gaps.update_one(
            {"skill_name": skill_name, "department_id": department_id},
            {"$set": gap.model_dump()},
            upsert=True
        )
    
    return list(skill_analysis.values())

# --- Resource Allocation ---
@api_router.post("/workforce/allocations")
async def create_resource_allocation(data: Dict[str, Any], current_user: User = Depends(get_current_user)):
...
async def get_resource_allocations(
    ...
    allocations = await db.resource_allocations.find(query, {"_id": 0}).sort("start_date", -1).to_list(500)
    return allocations

@api_router.get("/workforce/allocations/my")
async def get_my_allocations(current_user: User = Depends(get_current_user)):
    """Get current user's resource allocations"""
    emp = await db.employees.find_one({"user_id": current_user.id}, {"_id": 0})
    if not emp:
        return []
...
@api_router.put("/workforce/allocations/{allocation_id}")
async def update_resource_allocation(allocation_id: str, data: Dict[str, Any], current_user: User = Depends(get_current_user)):
    """Update a resource allocation"""
    data["updated_at"] = datetime.now(timezone.utc).isoformat()
    await db.resource_allocations.update_one({"id": allocation_id}, {"$set": data})
    
    allocation = await db.resource_allocations.find_one({"id": allocation_id}, {"_id": 0})
    return allocation

@api_router.delete("/workforce/allocations/{allocation_id}")
async def delete_resource_allocation(allocation_id: str, current_user: User = Depends(get_current_user)):
    """Delete a resource allocation"""
    result = await db.resource_allocations.delete_one({"id": allocation_id})
    if result.deleted_count == 0:
        raise HTTPException(status_code=404, detail="Allocation not found")
    return {"message": "Allocation deleted"}

# --- Capacity Planning ---
@api_router.post("/workforce/capacity-plans")
async def create_capacity_plan(data: Dict[str, Any], current_user: User = Depends(get_current_user)):
    """Create a capacity plan"""
    if current_user.role not in [UserRole.SUPER_ADMIN, UserRole.CORP_ADMIN]:
        raise HTTPException(status_code=403, detail="Only admins can create capacity plans")
    
    plan = CapacityPlan(**data, created_by=current_user.id)
    await db.capacity_plans.insert_one(plan.model_dump())
    return plan.model_dump()

@api_router.get("/workforce/capacity-plans")
async def get_capacity_plans(
    ...
    plans = await db.capacity_plans.find(query, {"_id": 0}).sort("period_start", -1).to_list(100)
    return plans

@api_router.get("/workforce/capacity-plans/{plan_id}")
async def get_capacity_plan(plan_id: str, current_user: User = Depends(get_current_user)):
    """Get a specific capacity plan"""
    plan = await db.capacity_plans.find_one({"id": plan_id}, {"_id": 0})
    if not plan:
        raise HTTPException(status_code=404, detail="Plan not found")
    return plan

@api_router.put("/workforce/capacity-plans/{plan_id}")
async def update_capacity_plan(plan_id: str, data: Dict[str, Any], current_user: User = Depends(get_current_user)):
    """Update a capacity plan"""
    if current_user.role not in [UserRole.SUPER_ADMIN, UserRole.CORP_ADMIN]:
        raise HTTPException(status_code=403, detail="Only admins can update capacity plans")
    
    data["updated_at"] = datetime.now(timezone.utc).isoformat()
    await db.capacity_plans.update_one({"id": plan_id}, {"$set": data})
    
    plan = await db.capacity_plans.find_one({"id": plan_id}, {"_id": 0})
    return plan

# --- Scenarios ---
@api_router.post("/workforce/scenarios")
async def create_workforce_scenario(data: Dict[str, Any], current_user: User = Depends(get_current_user)):
...
@api_router.get("/workforce/scenarios")
async def get_workforce_scenarios(
...
@api_router.get("/workforce/scenarios/{scenario_id}")
async def get_workforce_scenario(scenario_id: str, current_user: User = Depends(get_current_user)):
...
@api_router.put("/workforce/scenarios/{scenario_id}")
async def update_workforce_scenario(scenario_id: str, data: Dict[str, Any], current_user: User = Depends(get_current_user)):
    """Update a workforce scenario"""
    if current_user.role not in [UserRole.SUPER_ADMIN, UserRole.CORP_ADMIN]:
...
@api_router.delete("/workforce/scenarios/{scenario_id}")
async def delete_workforce_scenario(scenario_id: str, current_user: User = Depends(get_current_user)):
    """Delete a workforce scenario"""
    if current_user.role not in [UserRole.SUPER_ADMIN, UserRole.CORP_ADMIN]:
        raise HTTPException(status_code=403, detail="Only admins can delete scenarios")
    
    result = await db.workforce_scenarios.delete_one({"id": scenario_id})
    if result.deleted_count == 0:
        raise HTTPException(status_code=404, detail="Scenario not found")
    return {"message": "Scenario deleted"}

# --- Employee Availability (for employees to manage) ---
@api_router.get("/workforce/availability/my")
async def get_my_availability(
    ...
    availability = await db.employee_availability.find(query, {"_id": 0}).sort("date", 1).to_list(365)
    return availability

@api_router.post("/workforce/availability/my")
async def set_my_availability(data: Dict[str, Any], current_user: User = Depends(get_current_user)):
    """Set current user's availability for a date"""
    emp = await db.employees.find_one({"user_id": current_user.id}, {"_id": 0})
    if not emp:
        raise HTTPException(status_code=404, detail="Employee profile not found")
    
    data["employee_id"] = emp["id"]
    availability = EmployeeAvailability(**data)
    ...
    await db.employee_availability.update_one(
        ...
        {"$set": availability.model_dump()},
        upsert=True
    )
    
    return availability.model_dump()

@api_router.put("/workforce/availability/my/preferences")
async def update_my_preferences(data: Dict[str, Any], current_user: User = Depends(get_current_user)):
    """Update employee's project/role preferences"""
    emp = await db.employees.find_one({"user_id": current_user.id}, {"_id": 0})
    if not emp:
        raise HTTPException(status_code=404, detail="Employee profile not found")
    
    # Update employee record with preferences
    update_data = {}
    if "preferred_projects" in data:
        update_data["preferred_projects"] = data["preferred_projects"]
    if "interested_roles" in data:
        update_data["interested_roles"] = data["interested_roles"]
    if "open_to_relocation" in data:
        update_data["open_to_relocation"] = data["open_to_relocation"]
    if "open_to_travel" in data:
        update_data["open_to_travel"] = data["open_to_travel"]
...
async def get_workforce_dashboard(current_user: User = Depends(get_current_user)):
    ...
    # Get active headcount plans
    current_year = str(datetime.now().year)
    active_plans = await db.headcount_plans.find(
        {"fiscal_year": current_year, "status": {"$in": ["approved", "in_progress"]}},
        {"_id": 0}
    ).to_list(10)
    
    # Get critical skill gaps
    critical_gaps = await db.skill_gaps.find(
        {"priority": {"$in": ["critical", "high"]}},
        {"_id": 0}
    ).sort("gap_score", -1).to_list(10)
    
    # Get resource utilization
    allocations = await db.resource_allocations.find({"status": "active"}, {"_id": 0}).to_list(1000)
    
    total_allocated = sum(a.get("allocation_percentage", 0) for a in allocations)
    avg_utilization = total_allocated / len(allocations) if allocations else 0
    
    # Recent scenarios
    recent_scenarios = await db.workforce_scenarios.find(
        {},
        {"_id": 0}
    ...
    return {
        "summary": {
            "total_employees": total_employees,
            "active_headcount_plans": len(active_plans),
            "critical_skill_gaps": len(critical_gaps),
            "avg_utilization": round(avg_utilization, 1),
            "total_allocations": len(allocations)
        },
        "department_headcount": dept_headcount,
        "headcount_plans": active_plans,
        "skill_gaps": critical_gaps,
        "recent_scenarios": recent_scenarios
    }

# --- Employee Workforce View ---
@api_router.get("/workforce/my-overview")
async def get_my_workforce_overview(current_user: User = Depends(get_current_user)):
    """Get current employee's workforce overview"""
    emp = await db.employees.find_one({"user_id": current_user.id}, {"_id": 0})
    if not emp:
        return {
            "allocations": [],
    ...
    availability = await db.employee_availability.find(
        {"employee_id": emp["id"], "date": {"$gte": today, "$lte": end_date}},
        {"_id": 0}
    ).to_list(30)
    
    # Get skills
    skills = await db.employee_skills.find({"employee_id": emp["id"]}, {"_id": 0}).to_list(50)
    
    # Calculate utilization
    total_allocation = sum(a.get("allocation_percentage", 0) for a in allocations)
    
    return {
        "allocations": allocations,
        "availability": availability,
        "preferences": {
            "preferred_projects": emp.get("preferred_projects", []),
            "interested_roles": emp.get("interested_roles", []),
            "open_to_relocation": emp.get("open_to_relocation", False),
            "open_to_travel": emp.get("open_to_travel", False)
        },
        "skills": skills,
